...
export const updateMyAnimeListStatus = async (req, res, next) => {
  try {
    const { id } = req.user;
    const { anime_id } = req.params;
    const { status, score, num_watched_episodes, start_date, finish_date } = req.body;
    const data = await services.updateMyAnimeListStatus(id, anime_id, status, score, num_watched_episodes, start_date, finish_date);
    res.status(200).json({ ...data });
  } catch(err) {
    console.log("Error in the updateMyAnimeListStatus controller");
    next(err);
  }
}

export const deleteMyAnimeListItem = async (req, res, next) => {
  try {
    const { id } = req.user;
    const { anime_id } = req.params;
    const data = await services.deleteMyAnimeListItem(id, anime_id);
    res.status(200).json({ ...data });
  } catch(err) {
    console.log("Error in the deleteMyAnimeListItem controller");
    next(err);
  }
}

export const getUserAnimeList = async (req, res, next) => {
  try {
    const { id } = req.user;
    const { status, sort, limit, offset, fields } = req.query;
    const data = await services.getUserAnimeList(id, status, sort, limit, offset, fields);
    res.status(200).json({ ...data });
  } catch(err) {
    console.log("Error in the getUserAnimeList controller");
    next(err);
  }
}

export const getMyUserInformation = async (req, res, next) => {
  try {
    const { id } = req.user;
    const data = await services.getMyUserInformation(id);
    res.status(200).json({ ...data });
  } catch(err) {
    console.log("Error in the getMyUserInformation controller");
    next(err);
  }
}
